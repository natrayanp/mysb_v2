...
@app.route('/testfileup123',methods=['GET','POST','OPTIONS'])
def mytestfileup():
	if request.method=='OPTIONS':
		...
		file.save(os.path.join('/home/natrayan/project/AwsProject/Python/Tradingapp/tradingapp5/userregistration/', 'nat1.tiff'))
...
def create_fileupload_bse():
	if request.method=='OPTIONS':
		...
		fileup_resp = client.service.UploadFile({'Flag':flag,'UserId':settings.USERID[settings.LIVE],'EncryptedPassword':encpasswd,'MemberCode':membercode,'ClientCode':clientcode,'FileName':filename,'DocumentType':'','pFileBytes':fls,'Filler1':'Null','Filler2':'Null','_soapheaders':[header_value]})
