...
def testst():
    if request.method=='GET':
        ...
        command = cur.mogrify("SELECT lguserid,lgusername,lgusertype,lgentityid FROM userlogin;")
        print(command)
...
#This is called by setjws service
    if request.method=='OPTIONS':
        ...
        command = cur.mogrify("SELECT lguserid,lgusername,lgusertype,lgentityid FROM userlogin WHERE lguserid = %s AND lguserstatus NOT IN ('S','I','B');",(uid,) )
        print(command)
        cur, dbqerr = db.mydbfunc(con,cur,command)
        print(cur)
        print(dbqerr)
        ...
        if (cur.rowcount) == 0:
            ...
            lguserid,lgusername,lgusertype,lgentityid = records[0]
        ...
        natjwt =  jwt.encode({'uid': lguserid,'entityid':lgentityid,'username':lgusername,'exp': exp,'iat': iat, 'usertype':lgusertype,'skd':seccdid}, secretcode, algorithm='HS256')          
...
#This is called by setjws service
    if request.method=='OPTIONS':
        ...
        cred = credentials.Certificate('/home/natrayan/project/AwsProject/Python/Tradingapp/tradingapp5/loginservices/serviceAccountKey.json')
        ...
        if updtsignup == True:
            ...
            command = cur.mogrify("INSERT INTO notifimaster (nfmid,nfname,nfmuserid,nfmscreenid,nfmessage,nfmsgtype,nfmprocessscope,nfmnxtact,nfmnxtactmsg,nfmnxtactnavtyp,nfmnxtactnavdest,nfmstartdt,nfmoctime,nfmlmtime,nfmentityid) VALUES (%s,'pendingregistration',%s,'dashboard','please complete user registration','notifaction','P','Y','','NONE','NONE',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,%s);",(nfmid,lguserid,lgentityid,))
            cur, dbqerr = db.mydbfunc(con,cur,command)
            print(dbqerr['natstatus'])
            if cur.closed == True:
                if(dbqerr['natstatus'] == "error" or dbqerr['natstatus'] == "warning"):
