...
def notiprocess():
    ...
    if request.method=='OPTIONS':
        ...
        if screenid == 'signin':
            ...
            command = cur.mogrify("SELECT nfuuid,nfuuserid,nfuentityid FROM webapp.notifiuser WHERE nfustatus = 'P' AND nfprocessscope IN ('D','S') AND nfuuserid = %s AND nfuentityid = %s;" ,(userid,entityid,))
            ...
            command = cur.mogrify("UPDATE webapp.notifiuser SET nfulazyldid = %s WHERE nfuuserid = %s AND nfuentityid = %s AND nfuscreenid = %s;",(lazyloadid,userid,entityid,screenid,))
            ...
            cmdqry = "SELECT nfumid,nfuname,nfuuserid,nfuentityid FROM webapp.notifiuser WHERE nfustatus = 'P' AND nfprocessscope NOT IN ('D','S') AND nfuuserid = %s AND nfuentityid = %s"
            cmdqry = cmdqry + " UNION "
            cmdqry = cmdqry + "SELECT nfumid,nfuname,nfuuserid,nfuentityid FROM webapp.notifiuser WHERE nfustatus = 'P' AND nfprocessscope IN ('D','S') AND nfuprocesbypgldsvc = 'Y' AND nfuuserid = %s AND nfuentityid = %s;"
...
def cknotiusrup2dt(screenid,userid,entityid,con,cur):
    ...
    command = cur.mogrify("select distinct nfuoctime from webapp.notifiuser where nfuuserid = %s and nfuentityid = %s;",(userid,entityid,) )
    print(command)
    cur, dbqerr = db.mydbfunc(con,cur,command)
    print(cur)
    print(dbqerr)
    ...
    if rowcount > 0:
        ...
        datetimenf = datetime.now()- timedelta(2)
    else:
        pass
    ...
    if datetimenf.date() == datetime.utcnow().date():
        ...
        query = "INSERT INTO webapp.notifiuser (nfumid,nfuname,nfuuserid,nfuscreenid,nfumessage,nfumsgtype,nfprocessscope,nfuhvnxtact,nfunxtactmsg,nfunxtactnavtyp,nfunxtactnavdest,nfulazyldidstatus,nfustatus,nfuprocesbypgldsvc,nfutodayshowncount,nfuoctime,nfulmtime,nfuentityid) "
        ...
        query = query + "FROM webapp.notifimaster WHERE nfmuserid IN (%s,'ALL') AND nfmentityid = %s "
        query = query + "AND nfmlmtime > (SELECT MAX(nfuoctime) FROM webapp.notifiuser WHERE nfuuserid IN (%s,'ALL') AND nfuentityid = %s) )"
        command = cur.mogrify(query,(entityid,userid,entityid,userid,entityid,))
    else:
        print('inside else')
        #notification user table is old, create from master now
        command = cur.mogrify("DELETE FROM webapp.notifiuser WHERE nfuuserid = %s AND nfuentityid = %s",(userid,entityid,))
        print(command)
        cur, dbqerr = db.mydbfunc(con,cur,command)
        print(cur)
        print(dbqerr)
        ...
        query = "INSERT INTO webapp.notifiuser (nfumid,nfuname,nfuuserid,nfuscreenid,nfumessage,nfumsgtype,nfprocessscope,nfuhvnxtact,nfunxtactmsg,nfunxtactnavtyp,nfunxtactnavdest,nfulazyldidstatus,nfustatus,nfutodayshowncount,nfuoctime,nfulmtime,nfuentityid) "
...
def deletenotification(userid,entityid,name,uid,recordtodelete):
    ...
    if recordtodelete == 'both' or recordtodelete == 'master':
        command = cur.mogrify("DELETE FROM webapp.notifimaster WHERE nfmuserid = %s AND nfmentityid = %s AND nfname= %s AND nfmid = %s;" ,(userid,entityid,name,uid,))
        cur, dbqerr = db.mydbfunc(con,cur,command)
        print(dbqerr['natstatus'])
        if cur.closed == True:
            if(dbqerr['natstatus'] == "error" or dbqerr['natstatus'] == "warning"):
    ...
    if recordtodelete == 'both' or recordtodelete == 'notiuser':
        command = cur.mogrify("DELETE FROM webapp.notifiuser WHERE nfuuserid = %s AND nfuentityid = %s AND nfuname= %s AND nfumid = %s;" ,(userid,entityid,name,uid,))
        cur, dbqerr = db.mydbfunc(con,cur,command)
        print(dbqerr['natstatus'])
        if cur.closed == True:
            if(dbqerr['natstatus'] == "error" or dbqerr['natstatus'] == "warning"):
...
def updatenotificationstatus(userid,entityid,uid):
    ...
    command = cur.mogrify("UPDATE webapp.notifiuser SET nfustatus = 'C', nfulmtime = CURRENT_TIMESTAMP WHERE nfuuserid = %s AND nfuentityid = %s AND nfumid = %s;",(userid,entityid,uid,))                        
    cur, dbqerr = db.mydbfunc(con,cur,command)
    print(dbqerr['natstatus'])
    if cur.closed == True:
        if(dbqerr['natstatus'] == "error" or dbqerr['natstatus'] == "warning"):
...
def getuserstatus(userid,entityid):
    ...
    command = cur.mogrify("SELECT lguserstatus FROM webapp.userlogin WHERE lguserid = %s AND lgentityid = %s;" ,(userid,entityid,))
    cur, dbqerr = db.mydbfunc(con,cur,command)
    print(cur)
    print(dbqerr)
    print(type(dbqerr))
