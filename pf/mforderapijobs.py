...
#Looks like there is a API available for this
    ...
    '''
    for orderstatusrec in orderstatusres:
        status = orderstatusrec.get('status')
        orderstatus = 'PPP'
        order_id = orderstatusrec.get('order_id')
        segment = 'BSEMF'
        savetimestamp = datetime.now()
        pfsavetimestamp=savetimestamp.strftime('%Y-%m-%d %H:%M:%S')
        client_code = orderstatusrec.get('client_code')
        member_code = '17123'
        entity_id = 'IN'
...
def mforderallotschejb():
    ...
    """
        select json_agg(b) from (
            SELECT tran_entityid,tran_pfportfolioid,tran_producttype,tran_schemecd,count(1) as count FROM webapp.trandetails WHERE tran_dailypositionflg = 'N' AND mfor_entityid =%s 
            GROUP BY tran_entityid,tran_pfportfolioid,tran_producttype,tran_schemecd
            LIMIT 1000
        ) as b;
    """,(entityid,))
    print(command)
...
def process_one_pfrecord(criteria):
    ...
    while reloop:
        ...
        #Move the last but 2nd record (ie.4th records date) to history table.
        command = cur.mogrify(
        """
        ...
        if cur.closed == True:
            has_db_error = True
            if(dbqerr['natstatus'] == "error" or dbqerr['natstatus'] == "warning"):
                mm = prec_2nd_last_rec_date + " : daily position movement to history Failed" 
                if dbqerr['statusdetails'] == None or dbqerr['statusdetails'] == '':
                    dbqerr['statusdetails']=
                else:
                    dbqerr['statusdetails'] = dbqerr['statusdetails'] + "|" + mm
        ...
        if cur.closed == True:
            has_db_error = True            
            if(dbqerr['natstatus'] == "error" or dbqerr['natstatus'] == "warning"):
                mm = prec_last_rec['dpso_date'] + " : daily position INSERT Failed" 
                if dbqerr['statusdetails'] == None or dbqerr['statusdetails'] == '':
                    dbqerr['statusdetails'] = mm
                else:
                    dbqerr['statusdetails'] = dbqerr['statusdetails'] + "|" + mm
        ...
        for prec in positionrecs:
            ...
            if cur.closed == True:
                has_db_error = True
                if(dbqerr['natstatus'] == "error" or dbqerr['natstatus'] == "warning"):
                    mm = prec['dpso_date'] + " : daily position UPDATE in history table Failed" 
                    if dbqerr['statusdetails'] == None or dbqerr['statusdetails'] == '':
                        dbqerr['statusdetails'] = mm
                    else:
                        dbqerr['statusdetails'] = dbqerr['statusdetails'] + "|" + mm
