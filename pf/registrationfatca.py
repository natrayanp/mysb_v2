...
#This is called by setjws service
    if request.method=='OPTIONS':
        ...
        command = cur.mogrify("SELECT pan_rp,inv_name,tax_status,po_bir_inc,co_bir_inc,tax_res1,tpin1,id1_type,tax_res2,tpin2,id2_type,tax_res3,tpin3,id3_type,tax_res4,tpin4,id4_type,srce_wealt,inc_slab,pep_flag,occ_code,occ_type FROM webapp.fatcamaster WHERE fatcalguserid = %s AND fatcaentityid = %s;",(userid,entityid,))
        print(command)
        cur, dbqerr = db.mydbfunc(con,cur,command)
        print(cur)
        print(dbqerr)
...
#This is called when save for later button is clicked
    if request.method=='OPTIONS':
        ...
        comqry = 'UPDATE webapp.fatcamaster SET ('+ fatcakeys + ') = (' + fatcavals + ') WHERE fatcalguserid = %s AND fatcaentityid = %s;'
        command = cur.mogrify(comqry,(userid,entityid,))
        print('command :',command)                
...
def regisandfatcsubmit():
    ...
    if request.method=='OPTIONS':
        ...
        command = cur.mogrify("SELECT row_to_json(t) from(SELECT * FROM webapp.uccclientmaster A FULL OUTER JOIN fatcamaster B ON A.ucclguserid = B.fatcalguserid) as t where t.fatcalguserid = %s and t.fatcaentityid = %s AND t.ucclguserid = %s AND t.uccentityid = %s;",(userid,entityid,userid,entityid,))
        print(command)
        cur, dbqerr = db.mydbfunc(con,cur,command)
        print(cur)
        print(dbqerr)
        ...
        if r.status_code != 200:	
            ...
            command = cur.mogrify("UPDATE webapp.userlogin SET lguserstatus = 'U', lglmtime = CURRENT_TIMESTAMP WHERE lguserid = %s AND lgentityid = %s;",(userid,entityid,))        
            print(command)            
            cur, dbqerr = db.mydbfunc(con,cur,command)
            print(dbqerr['natstatus'])
            if cur.closed == True:
            ...
            print('nfmid :',nfmid)
            command = cur.mogrify("INSERT INTO webapp.notifimaster (nfmid,nfname,nfmuserid,nfmscreenid,nfmessage,nfmsgtype,nfmprocessscope,nfmnxtact,nfmnxtactmsg,nfmnxtactnavtyp,nfmnxtactnavdest,nfmstartdt,nfmoctime,nfmlmtime,nfmentityid) VALUES (%s,'pendingregisupload',%s,'dashboard','<div fxLayout=#column# fxLayoutWrap><div><p> Account Open Form (AOF) is sent to your email. Please upload the form to complete registration </p><p>Go to Setting > Docupload </div><div>','notifaction','P','Y','','NONE','NONE',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,%s) ON CONFLICT DO NOTHING;",(nfmid,userid,entityid,))
            print(command)
            cur, dbqerr = db.mydbfunc(con,cur,command)
            print(dbqerr['natstatus'])
            if cur.closed == True:
